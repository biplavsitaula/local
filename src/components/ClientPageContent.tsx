...
function PageContent() {
  ...
  return (
    <div className={`min-h-screen text-foreground transition-colors ${theme === 'dark'
      ? 'bg-background'
      : 'bg-gray-50'
      }`}>
      <Header
      ...
      <main className="flex flex-col gap-12">
        ...
        {showSeasonalSection && !isSeasonalHidden && seasonalTheme && (
          <section className={`mx-auto w-full max-w-6xl min-w-0 relative overflow-hidden rounded-3xl border ${seasonalTheme.colors.accent} p-8 px-4 md:px-8 transition-colors mb-4 ${theme === 'dark'
            ? 'gradient-card shadow-glow'
            : 'bg-white/90 border-orange-200/60 shadow-lg'
            }`}>
            <button
              onClick={handleCloseSeasonalSection}
              className={`absolute top-4 right-4 z-10 p-2 rounded-full border transition-colors cursor-pointer ${theme === 'dark'
