...
export function EditOfferModal({
  ...
  useEffect(() => {
    if (offer && open) {
      ...
      const offerData = (offer as any).data || offer;

      // Extract numeric values safely
      const getNumericString = (value: any): string => {
      ...
      setFormData({
        ...
        discountPercent: getNumericString(offerData.discountPercent),
        discountAmount: getNumericString(offerData.discountAmount),
        ...
        minPurchaseAmount: getNumericString(offerData.minPurchaseAmount),
        maxDiscountAmount: getNumericString(offerData.maxDiscountAmount),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await offersService.update(offerId, payload);
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <p className="text-sm text-muted-foreground">
            Editing: <span className="font-medium text-foreground">{offer.title}</span>
            {(offer.discountPercent || offer.discountAmount) && (
              <span className="ml-2">
                | Discount: {offer.discountPercent ? `${offer.discountPercent}%` : `Rs. ${offer.discountAmount}`}
              </span>
            )}
          </p>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <p className="text-xs text-muted-foreground -mb-2">
            Note: Provide either Discount Percent OR Discount Amount (not both)
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => {
                  // If entering percent, clear amount (only one discount type allowed)
                  const newValue = e.target.value;
                  setFormData({ 
                    ...formData, 
                    discountPercent: newValue,
                    discountAmount: newValue ? "" : formData.discountAmount
                  });
                  if (errors.discountPercent || errors.discountAmount) {
                    setErrors({ ...errors, discountPercent: "", discountAmount: "" });
                  }
                }}
                placeholder="e.g., 20"
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => {
                  // If entering amount, clear percent (only one discount type allowed)
                  const newValue = e.target.value;
                  setFormData({ 
                    ...formData, 
                    discountAmount: newValue,
                    discountPercent: newValue ? "" : formData.discountPercent
                  });
                  if (errors.discountPercent || errors.discountAmount) {
                    setErrors({ ...errors, discountPercent: "", discountAmount: "" });
                  }
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                <Input
                  ...
                  className="bg-background border-border text-base sm:text-base h-10 w-20 cursor-pointer"
                />
                <Input
                  type="text"
                  ...
                  onChange={(e) => {
                    ...
                    // Allow hex color format or empty
                    if (value === "" || /^#[0-9A-Fa-f]{0,6}$/.test(value)) {
                      setFormData({ ...formData, color: value });
                    }
                  }}
                  placeholder="Enter hex color (e.g., #FF5733)"
                  className="flex-1 bg-background border-border text-base sm:text-base"
                />
              </div>
              {formData.color ? (
                <p className="text-xs text-muted-foreground">Current: {formData.color}</p>
              ) : (
                <p className="text-xs text-muted-foreground">No color set (optional)</p>
              )}
            </div>
