...
import { Package, Minus, AlertCircle, Loader2, AlertTriangle } from "lucide-react";
...
export function RemoveStockModal({
  ...
  const validateForm = () => {
    ...
    if (
      selectedProduct &&
      parseInt(formData.quantity) > (selectedProduct.stock ?? 0)
    ) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await inventoryService.removeStock({
      ...
      toast.success(
        `Successfully removed ${formData.quantity} units from ${
  ...
  const newStock = selectedProduct
    ? Math.max(0, (selectedProduct.stock ?? 0) - parseInt(formData.quantity || "0"))
    : 0;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px] bg-card border-border">
        <DialogHeader>
          <DialogTitle className="text-xl font-display text-flame-red flex items-center gap-2">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-4">
          ...
          {selectedProduct && (
            <div className="p-3 bg-flame-red/10 border border-flame-red/20 rounded-lg">
              <div className="flex items-center gap-2">
                <Package className="h-4 w-4 text-flame-red" />
                <span className="font-medium text-flame-red">
          ...
          <div className="space-y-2">
            <Label htmlFor="quantity" className="text-foreground">
              Quantity to Remove <span className="text-flame-red">*</span>
            </Label>
            <Input
              id="quantity"
              type="number"
            ...
            {formData.quantity && selectedProduct && !errors.quantity && (
              <p className="text-xs text-muted-foreground">
                New stock will be:{" "}
                <span
                  className={cn(
                    "font-semibold",
                    newStock === 0 ? "text-flame-red" : "text-flame-orange"
                  )}
                >
                  {newStock}
                </span>{" "}
                units
                {newStock === 0 && (
                  <span className="text-flame-red ml-1">
          ...
          {selectedProduct &&
            formData.quantity &&
            parseInt(formData.quantity) > (selectedProduct.stock ?? 0) * 0.5 && (
              <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-lg flex items-start gap-2">
                <AlertTriangle className="h-4 w-4 text-amber-500 mt-0.5" />
                <p className="text-sm text-amber-500">
                  You are removing more than 50% of the current stock. Please
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              placeholder="e.g., Claim filed with insurance, Batch #XYZ123"
          ...
          <div className="flex gap-3 pt-4">
            ...
            <Button
              type="submit"
              className="flex-1 bg-flame-red hover:bg-flame-red/90 text-white"
              disabled={loading}
            >
              {loading ? (
                <>
